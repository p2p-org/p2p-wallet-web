...
export class Transaction implements Serializable<SerializableTransaction> {
    ...
    const instructions: SerializedInstruction[] | undefined = this.message?.instructions?.map(
      (instruction) => {
        ...
        if ((instruction as PartiallyDecodedInstruction).accounts) {
          (serializedInstruction as SerializedPartiallyDecodedInstruction).accounts = (
  ...
  static from(serializableTransaction: SerializableTransaction): Transaction {
    const accountKeys: ParsedMessageAccount[] | undefined =
      serializableTransaction.message?.accountKeys?.map((key) => ({
        ...key,
        pubkey: new PublicKey(key.pubkey),
      }));

    const instructions: Instruction[] | undefined =
      serializableTransaction.message?.instructions?.map((instruction) => {
        ...
        if ((instruction as SerializedPartiallyDecodedInstruction).accounts) {
          (originalInstruction as PartiallyDecodedInstruction).accounts = (
            instruction as SerializedPartiallyDecodedInstruction
